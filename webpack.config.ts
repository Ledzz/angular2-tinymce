...
if (!DEV_SERVER) {
	COPY_FOLDERS.unshift({ from: 'demo/index.html' });
} else {
...
const clientConfig = function webpackConfig(): WebpackConfig {
	...
	config.module = {
		rules: [
			...
			{ test: /\.html/, loader: 'raw-loader', exclude: [root('demo/index.html')] },
	...
	if (DLL) {
		...
		config.plugins.push(
			...
			new CopyWebpackPlugin([{ from: 'demo/assets/dist_root' }])
		);
	}
	...
	if (DLL) {
		...
		if (AOT) {
				config.entry = {
					main: './demo/main.browser.aot'
				};
			} else {
				config.entry = {
					main: './demo/main.browser'
				};
			}
	}
	if (!DLL) {
